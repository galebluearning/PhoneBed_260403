 ...
 * @brief LCD 背光 PWM 驱动板级支持包 (BSP) 源文件
...
{ 
  ledc_timer_config_t timer_conf = 
  {
    ...
    .freq_hz = 50 * 1000,
    .clk_cfg = LEDC_AUTO_CLK,  // 修改为AUTO_CLK，与其他定时器保持一致
  };
  ledc_channel_config_t ledc_conf = 
  {
    .gpio_num = EXAMPLE_PIN_NUM_BK_LIGHT,
    .speed_mode = LEDC_LOW_SPEED_MODE,
    .channel =  LEDC_CHANNEL_4,  // 使用通道4，避免与电机通道0-3冲突
...
{
  ESP_ERROR_CHECK_WITHOUT_ABORT(ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_4, duty));
  ESP_ERROR_CHECK_WITHOUT_ABORT(ledc_update_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_4));
}